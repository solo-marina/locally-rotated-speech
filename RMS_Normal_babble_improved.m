...
path2target = '/Users/marina-solo/Downloads/STAGE 2026 Solo/soundcheck';
path2out    = '/Users/marina-solo/Downloads/STAGE 2026 Solo/soundcheck_out';
...
for i_wav = 1:length(D_target)
    ...
    for i_band = 1:length(Bandscond)
        ...
        audiowrite([path2out filesep sentence_name '_LR_' num2str(Nbands) 'b_NB.wav'], S_LR_noisy, fs_s);
        disp(['  -> Saved LR ' num2str(Nbands) ' bands']);

        audiowrite([path2out filesep sentence_name '_NV_' num2str(Nbands) 'b_NB.wav'], S_NV_noisy, fs_s);
