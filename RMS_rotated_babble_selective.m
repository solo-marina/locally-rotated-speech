...
path2out    = '/Users/marina-solo/Downloads/STAGE 2026 Solo/Experiment 2/rotated babble SNR 13';
...
%% Scan CSVs for required LRB files
...
for c = 1:length(csvFiles)
    ...
    for col = 1:width(T)
        vals = string(T{:, col});
        if ~any(contains(vals, '_LRB'),'all'), continue; end
        matches = vals(endsWith(vals, '_LRB.mp3') | endsWith(vals, '_LRB.wav') | endsWith(vals, '_LRB'));
...
neededBase = neededFiles;
disp(['CSV scan: ' num2str(length(neededBase)) ' unique LRB files required.']);
...
for i_wav = 1:length(D_target)
    ...
    for i_band = 1:length(Bandscond)
        ...
        if any(strcmp(neededBase, [sentence_name '_NV_' num2str(Nbands) 'b_LRB'])) || ...
           any(strcmp(neededBase, [sentence_name '_LR_' num2str(Nbands) 'b_LRB']))
            anyNeeded = true;
            break
        end
    end
    ...
    for i_band = 1:length(Bandscond)
        ...
        outName_NV = [sentence_name '_NV_' num2str(Nbands) 'b_LRB'];
        outName_LR = [sentence_name '_LR_' num2str(Nbands) 'b_LRB'];
